...
export class InstallManager {
    ...
    public installPackages(idfToolsManager: IdfToolsManager,
                           progress: vscode.Progress<{message?: string, increment?: number}>): Promise<void> {
        return idfToolsManager.getPackageList()
            .then((packages) => {
                ...
                return utils.buildPromiseChain(packages, async (pkg) => {
                    ...
                    const absolutePath: string = this.getToolPackagesPath(["tools", pkg.name, versionName]);
                    await del(absolutePath , { force: true});

                    progress.report({message: `Installing ${count}/${packages.length}: ${pkg.description}...`});
                    ...
                    if (parsedUrl[parsedUrl.length - 1] === "zip") {
                        ...
                        p = new Promise<void>((resolve) => { resolve(); });
                    }
                    count += 1;
                    return p;
                });
        ...
        return new Promise(async (resolve, reject) => {
            await pathExists(zipFilePath).then((doesZipFileExists) => {
                ...
                yauzl.open(zipFilePath, { lazyEntries: true}, (error, zipfile) => {
                    if (error) {
                        return reject(new PackageError("Zip file error",
                        ...
                        await utils.dirExistPromise(absolutePath).then(async (dirExists) => {
                            if (dirExists) {
                                await del(absolutePath, { force: true}).catch((err) => {
                                    ...
                                    return reject(new PackageError("Install folder cant be deleted",
                                        "InstallZipFile", err, err.errorCode));
                                });
                            }
                        });
                                            ...
                                            writeStream.on("close", async () => {
                                                try {
                                                    ...
                                                    return reject(new PackageError(
                                                        ...
                                                        "InstallZipFile", closeWriteStreamErr));
                                                }
                                                zipfile.readEntry();
                                            });
                                            writeStream.on("error", (writeStreamErr) => {
                                                return reject(new PackageError(
                                                    "Error in writeStream", "InstallZipFile",
                                                    writeStreamErr));
                                            });
                    ...
                    await utils.validateFileSizeAndChecksum(packageFile, urlInfo.sha256, urlInfo.size)
                        .then(async (isValidFile) => {
                            if (isValidFile) {
                                ...
                                this.appendChannel(`Package ${pkg.description} downloaded file is invalid.`);
                                return reject(new PackageError("Downloaded file invalid", "InstallZipPackage"));
                            }
                    });
                } else {
                    this.appendChannel(`${pkg.description} downloaded file is not available.`);
                    return reject(new PackageError("Error finding downloaded file", "InstallZipPackage"));
                }
            });
        });
    }
                                ...
                                await utils.dirExistPromise(absolutePath).then(async (dirExists) => {
                                    if (dirExists) {
                                        await del(absolutePath, { force: true}).catch((err) => {
                                            ...
                                            return reject(new PackageError("Install folder cant be deleted",
                                                "installTarPackage", err, err.errorCode));
                                        });
                                    }
                                });
                                ...
                                await pathExists(binPath).then((exists) => {
                                    if (!exists) {
                                        ...
                                        extractor.on("error", (err) => {
                                            reject(new PackageError(
                                                "Extracting gunzipped tar error", "InstallTarPackage", err,
                                                err.code));
                                        });
                                        extractor.on("finish", () => {
                                            resolve();
                                        ...
                                        this.appendChannel(`Existing ${pkg.description} found in ${this.installPath}`);
                                        resolve();
                                    }
                                });
                            } else {
                                this.appendChannel(`Package ${pkg.description} downloaded file is invalid.`);
                                return reject(new PackageError("Downloaded file invalid", "InstallTarPackage"));
                            }
                        });
                } else {
                    this.appendChannel(`Package ${pkg.description} downloaded file not available.`);
                    return reject(new PackageError("Downloaded file unavailable", "InstallTarPackage"));
                }
            });
        });
    }
