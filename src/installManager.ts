        ...
        if (parsedUrl[parsedUrl.length - 1] === "zip") {
          p = this.installZipPackage(idfToolsManager, pkg).then(async () => {
            if (pkg.strip_container_dirs) {
              this.promisedStripContainerDirs(
                absolutePath,
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      if (packageDownloaded) {
        ...
        if (isValidFile) {
          ...
          try {
            await this.installZipFile(packageFile, absolutePath);
            return resolve();
          } catch (error) {
            return reject(error);
          }
          ...
          return reject(
            new PackageError("Downloaded file invalid", "InstallZipPackage")
          );
        }
      } else {
        this.appendChannel(
        ...
        return reject(
          new PackageError("Error finding downloaded file", "InstallZipPackage")
        );
      }
    });
  }
  ...
  private promisedStripContainerDirs(pkgDirPath: string, levels: number) {
    return new Promise<void>(async (resolve, reject) => {
      ...
      for (let i = 0; i < levels; i++) {
        ...
        if (files.length > 1) {
          return reject(`At level ${i} expected 1 entry, got ${files.length}`);
        }
