...
export class KconfigMenuLoader {
  ...
  public updateIdfPath() {
    ...
    const wrongPathMsg = this.locDic.localize(
      "menuconfig.wrongIdfPath",
      "Wrong IDF_PATH in workspace settings. Would you like to set it?"
    );
    vscode.window.showErrorMessage(wrongPathMsg, "Yes", "No").then(selected => {
      if (selected === "Yes") {
        const enterPathMessage = this.locDic.localize(
          "kconfigMenu.enterIdfPath",
          "Enter IDF_PATH Path"
        );
