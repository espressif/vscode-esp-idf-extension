...
const fileTag: string = "Localization Dictionary";
...
export class LocDictionary {
    ...
    if (fs.existsSync(locDirPath) && fs.existsSync(this.localizationFile)) {
      try {
        this.dictionary = JSON.parse(
          fs.readFileSync(this.localizationFile).toString()
        );
  ...
  public getSchemaParts(pathToUse: string): string[] {
    ...
    const parts = pathToUse
      .replace(path.join(extensionName, "i18n"), "")
      .split(/(?:\\|\/)/g);
    parts.splice(0, 2);
  ...
  public getDictionaryType() {
    const extensionName = __dirname.replace(path.sep + "dist", "");
    const dictionarySchema = JSON.parse(
      fs.readFileSync(path.join(extensionName, "schema.i18n.json")).toString()
    );
