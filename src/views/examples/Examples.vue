    ...
    <div id="examples-window">
                        ...
                        <ul class="examples">
                            <li v-for="item in exampleGroup" :key="item">
                                <p
                                    ...
                                    v-text="item.name"
                                    :class="{ selectedItem: storeSelectedExample.name === item.name }"
                                />
                            </li>
                        </ul>
                    </li>
            ...
            <div id="example-content">
                <div v-if="hasExampleDetail">
                    <button v-if="selectedExample.name !== ''" 
                        v-on:click="openExample(selectedExample)"
                        class="check-button">
                            Create project using example {{selectedExample.name}}
                    </button>
                </div>
                <div v-if="hasExampleDetail" id="exampleDetail" v-html="exampleDetail">
                </div>
            </div>
    </div>
</template>
...
export default class Examples extends Vue {
    ...
    public openExample(selectedExample: IExample) {
        this.storeOpenExample({ pathToOpen: selectedExample.path, name: selectedExample.name });
    }
