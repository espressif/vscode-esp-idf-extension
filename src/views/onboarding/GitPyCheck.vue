<template>
    <div id="git-py-check">
        ...
        <p v-if="gitVersion === 'Not found'">Please install <a href="https://git-scm.com/downloads">Git</a> and reload this window.</p>
        <br><br>
        ...
        <p v-if="pyVersionList && pyVersionList[0] === 'Not found'">Please install <a href="https://www.python.org/downloads">Python</a> and reload this window.</p>
        <div v-if="selectedPythonVersion === pyVersionList[pyVersionList.length - 1]">
            <br> <br>
            <label>Enter absolute python binary path to use. Example: {{winRoot}}/Users/name/python<span v-if="winRoot !== ''">.exe</span></label>
            <br> <br>
            <input type="text" class="text-size" v-model="manualPythonPath" placeholder="Enter your absolute python binary path here">
        </div>
        <br> <br>
        <router-link v-on:click.native="savePyBin" to='/download' class="check-button">Configure ESP-IDF</router-link>
    </div>
</template>
...
export default class GitPyCheck extends Vue {
    ...
    public savePyBin() {
        if (this.selectedPythonVersion === this.pyVersionList[this.pyVersionList.length - 1]) {
