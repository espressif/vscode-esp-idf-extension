            ...
            switch (message.command) {
                            ...
                            if (doesDirExists) {
                                ...
                                if (selected.title === "Yes") {
                                    await ensureDir(message.new_value).then(async () => {
                                        await idfConf.writeParameter("idf.toolsPath",
                                            message.new_value);
                                    });
                                } else {
                ...
                case "downloadEspIdfVersion":
                    if (message.selectedVersion && message.idfPath) {
                        ...
                        downloadInstallIdfVersion(idfVersion, message.idfPath).then(async () => {
                            await this.updateIdfToolsManager(path.join(message.idfPath, "esp-idf"));
                        });
                    }
                    break;
