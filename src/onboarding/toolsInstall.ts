...
export async function downloadToolsInIdfToolsPath(workingDir: string,
    ...
    return vscode.window.withProgress({
        ...
        const packagesProgress = await idfToolsManager.getPackageList().then((pkgs) => {
            return pkgs.map((pkg) => {
        ...
        Logger.info("");
        await downloadManager.downloadPackages(idfToolsManager, progress, packagesProgress).catch((reason) => {
        ...
        OutputChannel.appendLine("");
        Logger.info("");
        OutputChannel.appendLine("The following paths should be added to env PATH");
        ...
        OnBoardingPanel.postMessage({ command: "load_custom_paths",
                custom_vars: JSON.parse(exportVars), custom_paths: exportPaths });
    });
}
