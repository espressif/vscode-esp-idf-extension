...
export class ProjectConfigurationManager {
  ...
  private initialize(): void {
    ...
    try {
      ...
      if (!configContent || configContent.trim() === '') {
          Logger.warn(`Project configuration file is empty: ${this.configFilePath}`);
          this.configVersions = [];
          return;
      }
      ...
      if (this.configVersions.length > 0) {
          vscode.window.showInformationMessage(
              `Loaded ${this.configVersions.length} project configuration(s): ${this.configVersions.join(', ')}`
          );
      } else {
           Logger.info(`Project configuration file loaded but contains no configurations: ${this.configFilePath}`);
      }
        ...
        Logger.errorNotify(`Failed to parse project configuration file: ${this.configFilePath}`, error, "ProjectConfigurationmanager initialize");
  ...
  private async handleConfigFileChange(): Promise<void> {
    try {
      ...
      // Find added versions
      const addedVersions = currentVersions.filter(v => !this.configVersions.includes(v));
      
      // Find removed versions
      const removedVersions = this.configVersions.filter(v => !currentVersions.includes(v));
      ...
      if (currentSelectedConfig && currentVersions.includes(currentSelectedConfig)) {
        await this.updateConfiguration(currentSelectedConfig, configData[currentSelectedConfig]);
      } else {
    ...
    try {
      ...
      if (this.configVersions.length > 0) {
        ...
        if (currentSelectedConfig && this.configVersions.includes(currentSelectedConfig)) {
          ...
          await this.updateConfiguration(this.configVersions[0], configData[this.configVersions[0]]);
        }
      } else {
        this.setInvalidConfigurationStatus();
      }
    } catch (error) {
      vscode.window.showErrorMessage(`Error parsing newly created config file: ${error.message}`);
      this.setInvalidConfigurationStatus();
    }
  }
  ...
  private setInvalidConfigurationStatus(): void {
    ...
    const statusBarItemTooltip = "Invalid configuration path. Click to modify project configuration";
    ...
    this.statusBarItems["projectConf"] = createStatusBarItem(
      `$(${this.commandDictionary[CommandKeys.SelectProjectConfiguration].iconId}) ${statusBarItemName}`,
      statusBarItemTooltip,
      "espIdf.projectConf",
      99,
      this.commandDictionary[CommandKeys.SelectProjectConfiguration].checkboxState
    );
  }

  private async updateConfiguration(configName: string, configData: any): Promise<void> {
    ...
    this.statusBarItems["projectConf"] = createStatusBarItem(
      `$(${this.commandDictionary[CommandKeys.SelectProjectConfiguration].iconId}) ${configName}`,
    ...
    await getIdfTargetFromSdkconfig(this.workspaceRoot, this.statusBarItems["target"]);
  ...
  public async selectProjectConfiguration(): Promise<void> {
    try {
      const projectConfigurations = await getProjectConfigurationElements(this.workspaceRoot);
      
      if (!projectConfigurations || Object.keys(projectConfigurations).length === 0) {
      ...
      await this.updateConfiguration(option.target, projectConfigurations[option.target]);
    } catch (error) {
      vscode.window.showErrorMessage(`Error selecting configuration: ${error.message}`);
    }
  }
