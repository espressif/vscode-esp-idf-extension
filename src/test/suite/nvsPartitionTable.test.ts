...
suite("NVS PartitionTable Suite", () => {
  test("CSV2JSON mockdata test", async () => {
    ...
    await assert.equal(
      JSON.stringify(csv2Json(dataCSV)),
      JSON.stringify(JSON.parse(dataJSON))
    );
  });
  ...
  test("Row validation", async () => {
    ...
    await assert.equal(
      JSON.stringify(validateRows(JSON.parse(dataJSON))),
  ...
  test("Key field max character length validation", async () => {
    ...
    await assert.equal(
      isInValidRow(invalidRow),
      "Maximum key length is 15 characters"
    );
  });
  ...
  test("Value field over 4000 bytes for string encoding", async () => {
    ...
    await assert.equal(
      isInValidRow(invalidRow),
      "String value is limited to 4000 bytes"
    );
  });
  ...
  let no64Types = numberTypes.filter(
    (element) => element !== "u64" && element !== "i64"
  );
  let just64Types = numberTypes.filter(
    (element) => element == "u64" || element == "i64"
  );
  ...
  just64Types.forEach((i) => {
    test(`Value field invalid numbers for ${i}`, async () => {
      let invalidRow1 = {
        ...
        encoding: `${i}`,
        value: `${
          minValues[i] === 0 ? minValues[i] - 1 : minValues[i].minus(1)
        }`,
        error: "",
      };
      let invalidRow2 = {
