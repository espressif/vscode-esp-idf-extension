...
const defaultSettings: IKconfigSettings = { maxNumberOfProblems: 1000, useIDFKconfigStyle: false };
...
connection.onDidChangeConfiguration((change) => {
    if (hasConfigCapability) {
        ...
        globalSettings = (
            (change.settings.kconfigServer || defaultSettings)) as IKconfigSettings;
    }
...
async function validateKConfigDocument(kconfigDocument: TextDocument): Promise<void> {
    ...
    diagnostics.push(...getBlockDiagnosticsFor(menuPattern,
        endmenuPattern, kconfigDocument, ["menu", "endmenu"]));
    diagnostics.push(...getBlockDiagnosticsFor(choicePattern,
        endChoicePattern, kconfigDocument, ["choice", "enchoice"]));
    ...
    for (const line of lines) {
        ...
        while (backslashMatch !== null) {
            const diagnostic: Diagnostic = {
                ...
                range: {
                    end: kconfigDocument.positionAt(textPosition + backslashMatch.index + backslashMatch.input.length),
        ...
        while (badStartMatch !== null) {
            const diagnostic: Diagnostic = {
                ...
                range: {
                    end: kconfigDocument.positionAt(textPosition + badStartMatch.index + badStartMatch.input.length),
        ...
        if (hasTrailingSpace) {
            const diagnostic: Diagnostic = {
                ...
                range: {
                    end: kconfigDocument.positionAt(textPosition +
                        line.length),
                    start: kconfigDocument.positionAt(textPosition +
                        line.length - 1),
                },
...
function getTreeIndentDiagnostics(kconfigDocument: TextDocument) {
    ...
    for (const line of lines) {
        ...
        const openingMatches =
            (/^(\s*)\b(menu|choice|config|menuconfig|endmenu|endchoice|help|mainmenu)\b/g).exec(line);

        if (parentStack.size() > 0 && openingMatches &&
            (openingMatches[0].trim() === "menu" || openingMatches[0].trim() === "menuconfig" ||
             ...
             openingMatches[0].trim() === "config")) {
                ...
                } else if (parentStack.peek() === "config" || parentStack.peek() === "menuconfig") {
                    parentStack.pop();
                }
        } else if (parentStack.size() > 0 && openingMatches &&
                    (openingMatches[0].trim() === "endmenu" || openingMatches[0].trim() === "endchoice")) {
            while (parentStack.size() > 0) {
                const startingWord = parentStack.pop();
                if ((startingWord === "menu" && openingMatches[0].trim() === "endmenu") ||
        ...
        if (lineMatch && (parentStack.peek() !== "help" && lineMatch[1].length !== (indentSize * parentStack.size()) ||
                parentStack.peek() === "help" && lineMatch[1].length < (indentSize * parentStack.size()))) {
            const diagnostic: Diagnostic = {
                message: `Line ${lineNum} has ${lineMatch[1].length}.
                    Expect indent ${indentSize * parentStack.size()}`,
                range: {
                    end: kconfigDocument.positionAt(textPosition + lineMatch.index + lineMatch[1].length),
                    start: kconfigDocument.positionAt(textPosition),
                },
                severity: DiagnosticSeverity.Warning,
                source: `${problemSourceName}`,
        ...
        if (openingMatches) {
            if (openingMatches[0].trim() !== "endmenu" && openingMatches[0].trim() !== "endchoice") {
        ...
        if (keyMatch !== null && stringMatch === null) {
            const diagnostic: Diagnostic = {
                ...
                range: {
                    ...
                    start: kconfigDocument.positionAt(textPosition + keyMatch.index + keyMatch[1].length),
                },
                severity: DiagnosticSeverity.Warning,
                source: `${problemSourceName}`,
            };
...
function getBlockDiagnosticsFor(openingPattern: RegExp,
                                closingPattern: RegExp,
                                kconfigDocument: TextDocument,
                                blockName: string[]): Diagnostic[] {
    const diagnostics: Diagnostic[] = [];
    const openIndices = [];
    const closeIndices = [];
    const text = kconfigDocument.getText();
    ...
    for (let i = 0; i < (openIndices.length) ; i++) {
        ...
        for (const closingIndex of closeIndices) {
            if (closingIndex > openIndices[i] && closingIndex < openIndices[i + 1]) {
                isCloseFound = true;
                break;
            } else if (i === openIndices.length - 1 && closingIndex > openIndices[i]) {
                isCloseFound = true;
                break;
            }
        }
...
function getEmptyBlocksDiagnostics(kconfigDocument: TextDocument): Diagnostic[] {
    const diagnostics: Diagnostic[] = [];
    ...
    while (menuMatch !== null) {
        if (menuMatch[2] && menuMatch[2].length > 0 && menuMatch[2] === "\n") {
            const diagnostic: Diagnostic = {
                ...
                range: {
                    end: kconfigDocument.positionAt(menuMatch.index + `menu ""`.length
                        + menuMatch[1].length + menuMatch[2].length),
                    start: kconfigDocument.positionAt(menuMatch.index + menuMatch[1].length),
                },
                severity: DiagnosticSeverity.Error,
                source: `${problemSourceName}`,
            };
    ...
    while (choiceMatch !== null) {
        if (choiceMatch[2] && choiceMatch[2].length > 0 && choiceMatch[2] === "\n") {
            const diagnostic: Diagnostic = {
                message: `choice statement doesn't have any config options.`,
                range: {
                    end: kconfigDocument.positionAt(choiceMatch.index + `choice `.length +
                        choiceMatch[1].length + choiceMatch[2].length),
                    start: kconfigDocument.positionAt(choiceMatch.index + choiceMatch[1].length),
                },
                severity: DiagnosticSeverity.Error,
                source: `${problemSourceName}`,
            };
    ...
    while (ifBlockMatch !== null) {
        if (ifBlockMatch[2] && ifBlockMatch[2].length > 0 && ifBlockMatch[2] === "\n") {
            const diagnostic: Diagnostic = {
                message: `if statement doesn't have any config options.`,
                range: {
                    end: kconfigDocument.positionAt(ifBlockMatch.index + `if `.length +
                    ifBlockMatch[1].length + ifBlockMatch[2].length),
                    start: kconfigDocument.positionAt(ifBlockMatch.index + ifBlockMatch[1].length),
                },
                severity: DiagnosticSeverity.Error,
                source: `${problemSourceName}`,
            };
...
connection.onCompletion((kconfigDocumentPosition: TextDocumentPositionParams) => {
    return [
        {
            data: 1,
    ...
    (item: CompletionItem): CompletionItem => {
        switch (item.data) {
            ...
            case 24:
                ...
                item.documentation =
                    `This defines a choice group <symbol> accepting config or menuconfig as options.
