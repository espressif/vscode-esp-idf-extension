...
export class ExamplesPlanel {
    ...
    public static createOrShow(extensionPath: string) {
        ...
        if (ExamplesPlanel.currentPanel) {
            ...
            ExamplesPlanel.currentPanel = new ExamplesPlanel(extensionPath, column || vscode.ViewColumn.One);
        }
    }
    ...
    private constructor(extensionPath: string, column: vscode.ViewColumn) {
        const onBoardingPanelTitle = locDic.localize("examples.panelName", "ESP-IDF Examples");
    ...
    private obtainExamplesList() {
        ...
        const exampleListInfo = examplesListPaths.map((examplePath) => {
            const exampleCategory = examplesCategories.find((exampleCat) => examplePath.indexOf(exampleCat) > -1);
            const regexToUse = process.platform === "win32" ? /([^\\]*)\\*$/ : /([^\/]*)\/*$/;
        ...
        this.panel.webview.postMessage({ command: "set_examples_path", example_list: exampleListInfo });

        this.panel.webview.postMessage({ command: "set_initial_example", selected_example: exampleListInfo[0]});
    }
}
