...
首先在 Visual Studio Code 中打开一个 ESP-IDF 项目。您可以按照 :ref:`创建 ESP-IDF 项目 <create_an_esp-idf_project>` 中的说明创建新的 ESP-IDF 项目，或打开已有的 ESP-IDF 项目。
...
要启动调试，请从菜单选择 ``运行`` > ``启动调试`` 或按 F5。

.. image:: ../../media/tutorials/debug/init_halted.png

您可以在调试控制台中查看 GDB 输出，在 ``查看`` > ``输出`` 菜单下选择下拉列表中的 ``ESP-IDF`` 可查看 OpenOCD 输出。
...
1.  首先，OpenOCD 服务器在后台启动，输出显示在菜单 ``查看`` > ``输出`` 中，从下拉菜单选择 ``ESP-IDF`` 即可查看。
...
在 Visual Studio Code 中选择菜单 **查看** > **输出** > **ESP-IDF**。该输出有助于了解扩展的当前状态，例如 ``OpenOCD`` 的通信会显示在此输出中。
...
如果修改了应用程序镜像偏移量，需要相应更新 OpenOCD 启动参数。当 OpenOCD 输出（菜单 ``查看`` > ``输出`` > ``ESP-IDF``）出现如下错误时即需更新：
...
要更新 OpenOCD 启动参数，请打开项目的 ``.vscode/settings.json`` 并添加或修改：
...
除 ESP-IDF 扩展外，您也可以使用其他扩展调试项目。例如，可使用 `Microsoft C/C++ <https://marketplace.visualstudio.com/items?itemName=ms-vscode.cpptools>`_ 扩展进行调试。

为此，需要在项目的 .vscode 目录中配置 launch.json 文件。以下为 launch.json 示例：
...
请注意，若使用这些扩展的调试配置，需要手动通过 ESP-IDF VS Code 扩展的 [OpenOCD] 状态栏按钮或终端启动 OpenOCD。
