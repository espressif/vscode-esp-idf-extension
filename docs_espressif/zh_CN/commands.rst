...
.. list-table::
   ...
   * - 创建项目配置
     - 在项目根目录中创建 ``CMakePresets.json``，其中包含 **default** 和 **production** 两个起始配置预设，可重命名并扩展。已定义的预设不会被修改。
   * - 清理当前 SDK 配置编辑器服务器进程
   ...
   * - 选择项目配置
     - 选择 ``CMakePresets.json`` 或 ``CMakeUserPresets.json`` 中用于覆盖扩展设置的配置预设。标记为 ``hidden`` 的预设以及被 ``condition`` 禁用的预设不会显示。
   * - 选择配置存储位置
